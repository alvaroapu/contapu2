...
import { Loader2, Mail, CheckCircle2, XCircle, AlertCircle, Search, Send } from 'lucide-react';
...
const DEFAULT_INTRO = (year: number) =>
  `Estimado/a {autor},\n\nNos complace comunicarle que hemos procesado el pago correspondiente a la liquidación del año ${year}.\n\nEn los próximos días recibirá el importe en la cuenta bancaria que nos facilitó.`;

const DEFAULT_OUTRO = () =>
  `Si tiene alguna duda sobre el pago, no dude en ponerse en contacto con nosotros.\n\nUn cordial saludo,\nApuleyo Ediciones`;

export function SendPaymentConfirmationDialog({ open, onOpenChange, liquidation, allItems }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    setSubject(`Confirmación de pago - Liquidación ${liquidation.year} - Apuleyo Ediciones`);
  ...
  const loadAuthors = async () => {
    ...
    const { data: logData } = await (supabase as any)
      .from('payment_confirmation_email_log')
  ...
  const sendEmail = async (authorData: AuthorEmail, idx: number, batchNum?: number): Promise<boolean> => {
    ...
    try {
      ...
      (supabase as any).from('payment_confirmation_email_log').upsert({
      ...
      <DialogContent className="w-[95vw] max-w-5xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Confirmar pago liquidación {liquidation.year} por email
          </DialogTitle>
        </DialogHeader>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="message" className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="from-email-conf" className="text-sm font-medium">
                    Email remitente
                ...
                <div>
                  <Label htmlFor="subject-conf" className="text-sm font-medium">Asunto</Label>
                  <Input
                    id="subject-conf"
                    value={subject}
              ...
              <div>
                <Label htmlFor="intro-conf" className="text-sm font-medium">
              ...
              <div>
                <Label htmlFor="outro-conf" className="text-sm font-medium">
                  Texto de cierre
                </Label>
                <Textarea
                  id="outro-conf"
              ...
              <p className="text-xs text-muted-foreground">
                💡 Usa <code className="bg-muted px-1 rounded">{'{autor}'}</code> para insertar el nombre del autor automáticamente. Este correo se envía sin archivos adjuntos.
              </p>
            </TabsContent>
          ...
          <div className="space-y-3 w-full">
            <div className="flex items-end gap-2">
              <div className="flex-1">
                <Label htmlFor="test-email-conf" className="text-xs text-muted-foreground">
            ...
            <div className="flex justify-end gap-2">
              ...
              {sending ? (
                ...
                <Button onClick={handleSendAll} disabled={sendableAuthors.length === 0 || sentCount === sendableAuthors.length}>
                  Confirmar pago a {sendableAuthors.filter(a => a.status !== 'sent').length} autor(es)
                </Button>
              )}
            </div>
          </div>
